 ...
 *
 * Every upload -- the whole catalog, replacing what came before -- becomes
 * its own row in `collections` plus its own `recipes` and `chunks` rows, so a
 * bad upload cannot damage the catalog the chat side is currently serving and
 * rolling back is a one-line flip of app_state.active_collection_id.
 *
...
/**
 * The summary row the admin table renders -- cheap, no recipes, chunks or
 * vectors touched.
 */
 ...
 * it rejected. The recipes are their own table, read with readRecipes().
 *
...
export async function createCollection({ name, format, chunkStrategy = 'single' }) {
  ...
  await query(
    ...
    [
      ...
      format,
      chunkStrategy,
      JSON.stringify({ rows: 0, recipes: 0, rejected: 0, chunks: 0 }),
    ]
  );
  return id;
}
 ...
 *
 * `recipes` is accepted here because the pipeline writes the recipes and the
 * row counts in one step: they go to the recipes table, everything else to
 * columns on the collection.
 */
export async function updateManifest(id, patch) {
  const { recipes, ...meta } = patch;
  ...
  await withTransaction(async (client) => {
    ...
    if (recipes) await replaceRecipes(client, id, recipes);
  });
...
async function replaceRecipes(client, id, recipes) {
  await client.query('DELETE FROM recipes WHERE collection_id = $1', [id]);
  await insertRows(
    client,
    'INSERT INTO recipes (collection_id, recipe_id, ord, data) VALUES',
    recipes.map((recipe, i) => [id, recipe.id, i, JSON.stringify(recipe)]),
    4
  );
}

/** The recipes of a collection, in upload order. */
export async function readRecipes(id) {
  if (!isValidId(id)) return [];
  const { rows } = await query(
    'SELECT data FROM recipes WHERE collection_id = $1 ORDER BY ord',
    [id]
  );
...
/**
 * The haystack the admin stock search matches against: title, cuisine, then
 * every diet tag and main ingredient, space-joined -- the same string the
 * previous in-memory filter built.
 */
const RECIPE_HAYSTACK = `
  lower(
    coalesce(data->>'title', '') || ' ' ||
    coalesce(data->>'cuisine', '') ||
    coalesce((SELECT ' ' || string_agg(value, ' ')
                FROM jsonb_array_elements_text(jsonb_list(data, 'dietTags'))), '') ||
    coalesce((SELECT ' ' || string_agg(value, ' ')
                FROM jsonb_array_elements_text(jsonb_list(data, 'mainIngredients'))), '')
  )
`;
 ...
 *
 * position() rather than LIKE: an owner's query can contain % or _ and those
 * are plain characters here, not wildcards.
 */
export async function searchRecipes(id, { q = '', offset = 0, limit = 25 } = {}) {
  if (!isValidId(id)) return { total: 0, matched: 0, recipes: [] };

  const needle = q.trim().toLowerCase();
  const where = needle
    ? `collection_id = $1 AND position($2 IN ${RECIPE_HAYSTACK}) > 0`
    : 'collection_id = $1';
  const params = needle ? [id, needle] : [id];

  const [totals, page] = await Promise.all([
    query(
      `SELECT (SELECT count(*) FROM recipes WHERE collection_id = $1) AS total,
              (SELECT count(*) FROM recipes WHERE ${where}) AS matched`,
      params
    ),
    query(
      `SELECT data FROM recipes WHERE ${where}
  ...
  return {
    ...
    recipes: page.rows.map((r) => r.data),
  };
}
...
export async function saveChunks(id, chunks) {
  await withTransaction(async (client) => {
    ...
    await insertRows(
      client,
      'INSERT INTO chunks (collection_id, chunk_id, recipe_id, ord, kind, text, hash) VALUES',
      chunks.map((c, i) => [id, c.chunkId, c.recipeId, i, c.kind, c.text, textHash(c.text)]),
      7
    );
  });
}
...
export async function readChunks(id, { offset = 0, limit = 20 } = {}) {
  if (!isValidId(id)) return [];
  const { rows } = await query(
    `SELECT chunk_id, recipe_id, kind, text FROM chunks
 ...
 *
 * This is what keeps re-uploading the full catalog cheap. A catalog that adds
 * ten recipes to a five-hundred-recipe spreadsheet and uploads the whole thing
 * again pays to embed ten chunks, not five hundred -- the other rows hash the
 * same and their vectors carry straight over.
 *
 * Hashing the text is the load-bearing part: a chunk id is derived from the
 * recipe id, so an edited instructions field keeps its chunk id while very
 * much needing a fresh vector. Matching on ids alone would silently serve a
 * stale embedding. The source collection must also have been built with the
 * same model and dimensionality, or its vectors are not comparable with the
 * new ones.
 *
...
/** How many chunks of a collection carry a usable vector AND a matching recipe. */
export async function countUsableEntries(id) {
  const { rows } = await query(
    `SELECT count(*) FILTER (WHERE c.embedding IS NOT NULL) AS embedded,
            count(*) FILTER (WHERE c.embedding IS NOT NULL AND r.recipe_id IS NOT NULL) AS usable
       FROM chunks c
       LEFT JOIN recipes r ON r.collection_id = c.collection_id AND r.recipe_id = c.recipe_id
...
export async function deleteCollection(id) {
  ...
  if ((await activeCollectionId()) === id) {
    const err = new Error(
      'Cannot delete the catalog that is currently serving cooks. Upload a ' +
