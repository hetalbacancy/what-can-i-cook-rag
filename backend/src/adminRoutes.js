...
const CSV_COLUMNS = [
  'id', 'title', 'cuisine', 'dietTags', 'mainIngredients',
  'optionalIngredients', 'timeMinutes', 'servings', 'instructions',
];
...
function toCsv(recipes) {
  const lines = [CSV_COLUMNS.join(',')];
  for (const recipe of recipes) lines.push(CSV_COLUMNS.map((c) => csvCell(recipe[c])).join(','));
  return lines.join('\n');
}
...
adminRouter.get('/recipes', async (req, res, next) => {
  try {
    ...
    if (!id) {
      return res.json({ collectionId: null, total: 0, matched: 0, offset: 0, limit, recipes: [] });
    }

    const offset = Math.max(0, Number(req.query.offset) || 0);
    const q = String(req.query.q ?? '');
    const { total, matched, recipes } = await searchRecipes(id, { q, offset, limit });

    res.json({ collectionId: id, total, matched, offset, limit, recipes });
  } catch (err) {
    next(err);
  }
});
 ...
 */
adminRouter.get('/recipes/export', async (req, res, next) => {
  try {
    const id = activeInfo()?.id;
    if (!id) return res.status(404).json({ error: 'No catalog is loaded yet' });

    const [recipes, manifest] = await Promise.all([readRecipes(id), readManifest(id)]);
