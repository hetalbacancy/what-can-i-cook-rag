 ...
 *
 * Every upload is the WHOLE catalog: the file the admin sends is the
 * kitchen's current recipe box in full, and it replaces whatever came
 * before. Adding a recipe, fixing an ingredient list and removing a
 * retired dish are all the same operation -- edit the spreadsheet, upload
 * it again. There is no partial-update path to get out of sync with.
 *
...
import { chunkRecipes, DEFAULT_STRATEGY } from './chunk.js';
import {
  activeCollectionId,
...
export async function startIngest({ name, format, chunkStrategy = DEFAULT_STRATEGY, buffer }) {
  ...
  runIngest({ job, collectionId, format, baseId, buffer }).catch((err) => {
    job.status = 'failed';
...
async function runIngest({ job, collectionId, format, baseId, buffer }) {
  ...
  await stage(job, 'validate', (entry) => {
    entry.count = parsed.recipes.length;
  ...
  const chunks = await stage(job, 'chunk', (entry) => {
    const result = chunkRecipes(parsed.recipes);
  ...
  // Recipes are their own rows; chunks carry the text retrieval scores against.
  ...
  job.stage = null;
  console.log(
    `[ingest] ${collectionId} ready -- ${parsed.recipes.length} recipes, ${chunks.length} chunks ` +
