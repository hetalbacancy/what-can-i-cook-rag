 ...
 *
 * The collection registry, the parsed recipes, and the chunks with their
 * vectors are four tables here. The uploaded file is still the source of
 * truth for a catalog; Postgres is where the processed form of it lives.
 *
 * The vectors are stored in a pgvector column and compared by the database,
 * so the similarity scan is a query rather than a loop in Node.
 */
...
const CONNECTION =
  process.env.DATABASE_URL || 'postgres://bacancy:devpass@localhost:5432/cookbot';
 ...
 * and at catalog sizes here an exact scan in Postgres is already
 * sub-millisecond. Revisit both decisions together somewhere north of ~100k
 * chunks.
 */
const SCHEMA = `
-- A recipe's dietTags/mainIngredients/optionalIngredients are JSON arrays,
-- but a hand-edited catalog can leave one holding a scalar or nothing at
-- all. jsonb_array_elements_text() errors on those, so every query that
-- expands a list field goes through this instead and gets an empty array
...
CREATE INDEX IF NOT EXISTS recipes_order_idx ON recipes (collection_id, ord);
...
CREATE INDEX IF NOT EXISTS chunks_recipe_idx ON chunks (collection_id, recipe_id);

-- One row, holding which collection is currently serving cooks. The CHECK
