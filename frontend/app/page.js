...
const GREETING = {
  role: 'model',
  text:
    "Hi, I'm **ChefMind**. Tell me what ingredients you've got on hand — a few staples, whatever's in the fridge — and I'll find recipes from my catalog that make the most of them.",
};

const STARTERS = [
  'I have chicken, garlic and rice',
  'Chickpeas, tomato and onion, something vegan',
  'Beef, tortillas and cilantro, quick dinner',
  'Just eggs, rice and soy sauce, 20 minutes tops',
  'Salmon and ginger, something gluten-free',
  'Lentils, carrot and onion for a cold night',
];
...
function Sources({ sources, searchQuery, userMessage }) {
  ...
  return (
    <details className="retrieved">
      ...
      <ol>
        {sources.map((s) => {
          // timeMinutes and dietTags are optional columns, so build the detail
          // list from whatever this recipe actually has -- otherwise a sparse
          // row renders as "(gluten-free, m, )".
          const detail = [
            s.timeMinutes ? `${s.timeMinutes}m` : null,
            s.servings ? `serves ${s.servings}` : null,
            s.dietTags?.slice(0, 2).join('/') || null,
          ].filter(Boolean);

          return (
            <li key={s.id}>
              <span className="sim">{Math.round(s.score * 100)}% match</span> — {s.title}
              {s.cuisine ? ` (${s.cuisine})` : ''}
...
export default function Home() {
  ...
  const [catalog, setCatalog] = useState({ count: 0, cuisines: [], dietTags: [] });
  ...
  useEffect(() => {
    ...
    // hardcoded strings, which would otherwise drift from backend/.env silently.
    fetch('/api/health')
      .then((r) => r.json())
      .then(setHealth)
      .catch(() => {});
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: question,
          history,
          filters: { cuisine: cuisine || undefined, dietTag: dietTag || undefined },
        }),
      });
    ...
    <div className="shell">
      <header className="masthead">
        <div className="brand">
          ...
          <div>
            <h1>ChefMind</h1>
            <p className="tagline">Retrieval-augmented recipe recommendations</p>
          </div>
        </div>
        <div className="pipeline">
          ...
          <span>→ {health?.chunks ?? 0} chunks</span>
          <span>→ cosine top-{health?.retrieveK ?? 12}</span>
          <span>→ overlap top-{health?.rerankTopK ?? 6}</span>
          <span>→ {health?.chatModel ?? 'gemini'}</span>
        </div>
        <p className="lede">
          {catalog.count ? (
            <>
              Every suggestion is grounded in a {catalog.count}-recipe catalog — open “Retrieved
        ...
        <main className="card chat">
          <div className="messages">
            {messages.map((m, i) => (
              <div key={i} className={`row ${m.role}`}>
                <div className="bubble">
                  <div className="who">{m.role === 'user' ? 'You' : 'ChefMind'}</div>
                  <div
            ...
            {busy && (
              <div className="row model">
                <div className="bubble">
                  <div className="who">ChefMind</div>
          ...
          <div className="composer">
            <textarea
              rows={1}
              value={input}
              placeholder="What have you got in the kitchen?"
        ...
        <aside className="sidebar">
          <section className="card">
            ...
            <div className="status">
              ...
              {catalog.count ? `${catalog.count} recipes indexed` : 'No catalog available yet'}
            </div>
            {catalog.collection && (
